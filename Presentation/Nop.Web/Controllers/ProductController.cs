...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual ActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            {
                var user = _customerService.GetCustomerByEmail(User.Identity.Name);
                IOrganizationService service = InitCRM();
                ...
                if (service != null)
                {
                    ...
                    Lead["subject"] = $"{product.Name}";
                    Lead["firstname"] = user.GetFullName();
                    Lead["fullname"] = user.GetFullName();
                    Lead["emailaddress1"] = user.Email;
                    service.Create(Lead);
